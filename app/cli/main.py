...
@click.option('--recommendation-profile', type=click.Choice(['steady_20p_10d', 'steady_20p_10d_v2', 'steady_20p_20d']), default=settings.active_recommendation_profiles[0] if settings.active_recommendation_profiles else 'steady_20p_10d', show_default=True, help='Recommendation profile for screening horizon')
@click.option('--batch-size', default=settings.batch_size, type=int, help='Batch processing size')
@click.option('--lookback-days', default=settings.lookback_days, type=int, help='Days of historical data')
def run(
    validate: bool,
    load_stocks: bool,
    load_prices: bool,
...
@models.command()
@click.option('--horizon', type=int, default=10, help='Prediction horizon in trading days')
def train(horizon: int):
    """Train the XGBoost stock direction model."""
    from app.services.modeling.train_model import train_xgboost_model
    
    click.echo(f"🧠 Training XGBoost model for {horizon}-day horizon...")
    db = get_session()
    try:
        train_xgboost_model(db, horizon_days=horizon)
        click.echo(f"✅ Training completed successfully for {horizon}-day model!")
