...
def run_pipeline(**context) -> dict:
    ...
    """
    execution_date = context.get("ds")  # YYYY-MM-DD format
    run_id = context.get("run_id")
    
    logger.info(f"Starting Nigerian Stock Pipeline for {execution_date}")
...
def generate_daily_summary(**context) -> str:
    ...
    Duration: {metrics['total_duration']:.2f} seconds ({metrics['total_duration']/60:.2f} minutes)
...
@dag(
    ...
    description="Daily ETL pipeline for Nigerian Stock Exchange data with technical analysis",
...
def nigerian_stock_pipeline_dag():
    ...
    run_etl_pipeline = PythonOperator(
        ...
        Executes all 7 stages of the Nigerian stock pipeline:
        ...
        7. **Evaluate Alerts**: Check 5 rule types, generate notifications
    ...
    check_sla = PythonOperator(
        ...
        provide_context=True,
        trigger_rule="all_done",  # Run even if pipeline fails
        doc_md="""
        ...
        **Trigger**: Runs regardless of pipeline success/failure
        """,
    )
